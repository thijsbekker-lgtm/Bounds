...
export async function loadInsightStats(sb,userId){
  const {data:rounds,error:roundError}=await sb.from('rounds').select('id,played_at,holes_played,course:courses(name),players:round_players!inner(id,user_id,final_score,stableford)').eq('owner_id',userId).eq('round_players.user_id',userId).order('played_at',{ascending:false}).limit(100);
  if(roundError) throw roundError;
  const rows=rounds||[];
  const playerIds=rows.flatMap(r=>(r.players||[]).map(p=>p.id));
  if(!playerIds.length) return {rounds:rows,holes:[]};
  const {data:holes,error:holeError}=await sb.from('hole_scores').select('round_player_id,score,stableford,putts,penalty,fairway,gir,course_hole:course_holes(hole_number,par,stroke_index)').in('round_player_id',playerIds);
...
async function loadPlayableHolesForRound(sb,tee){
  ...
  if(Number(tee.holes)===18 && Number(tee.physical_holes)===9){
    ...
    if(source){const physical=await loadHoles(sb,source.id);if(physical.length===9){return [...physical.map(h=>({...h,played_hole_number:h.hole_number})),...physical.map(h=>({...h,id:`${h.id}-2`,hole_number:h.hole_number+9,played_hole_number:h.hole_number+9,stroke_index:Number(h.stroke_index)+1}))];}}
  }
  return [];
}
...
export async function loadRoundDetail(sb,userId,roundId){
  ...
  const tee=await loadRoundTee(sb,data.tee_id); if(!tee) throw new Error('Tee van deze ronde niet gevonden.');
  ...
  if(he) throw he;
  const notes={};
  if(hs?.length){const ids=hs.map(x=>x.id);const {data:ns,error:ne}=await sb.from('hole_score_notes').select('hole_score_id,note').in('hole_score_id',ids);if(ne) throw ne;(ns||[]).forEach(n=>{notes[n.hole_score_id]=n.note||''});}
  const savedByPlayedHole=new Map((hs||[]).map(h=>[Number(h.played_hole_number ?? h.course_hole?.hole_number),h]));
  const holes=courseHoles.slice(0,Number(data.holes_played)).map(h=>{const playedHole=Number(h.played_hole_number ?? h.hole_number);const saved=savedByPlayedHole.get(playedHole);return {hole:playedHole,par:Number(h.par)||0,si:Number(h.stroke_index)||0,score:saved?.score ?? 0,sf:saved?.stableford ?? null,putts:saved?.putts ?? '',penalty:saved?.penalty ?? '',fairway:saved?.fairway ?? '',gir:saved?.gir ?? '',note:saved ? (notes[saved.id]||'') : ''};});
...
export async function saveRound(sb,payload){
  const {data,error}=await sb.rpc('save_round_v2',payload);
  if(error) throw error; return data;
}
