...
async function deleteSavedRound(roundId){const ok=confirm('Deze ronde verwijderen? De ronde en alle bijbehorende scores en statistieken worden definitief verwijderd.');if(!ok)return;try{const deleted=await data.deleteRound(sb,roundId);if(!deleted){toast('Ronde niet gevonden of je hebt geen rechten om deze te verwijderen.');return}toast('Ronde verwijderd.');await refreshDashboard();showTab('game')}catch(e){console.error(e);toast('Verwijderen mislukt: '+(e.message||'controleer je login'))}}
function roundHistoryItem(r){return `<div class="round-item round-history-item"><button class="round-button" data-round="${r.id}"><div><b>${esc(r.course?.name||'Baan')}</b><small>${dateLabel(r.played_at)} · ${r.holes_played} holes</small></div><div><b>${r.players?.[0]?.final_score||'—'}</b><small>${r.players?.[0]?.stableford||'—'} SF</small></div></button><button class="text-button danger-button" type="button" data-delete-round="${r.id}">Verwijder</button></div>`}
async function renderGame(){if(!user)return;const history=await data.loadHistory(sb,user.id);const scores=history.flatMap(r=>r.players||[]).map(p=>Number(p.final_score)).filter(Number.isFinite),sf=history.flatMap(r=>r.players||[]).map(p=>Number(p.stableford)).filter(Number.isFinite);$('#gameHcp').textContent=fmt(profile?.handicap_index);$('#gameRounds').textContent=history.length;$('#gameAvg').textContent=scores.length?fmt((scores.reduce((a,b)=>a+b,0)/scores.length).toFixed(1)):'—';$('#gameBestSf').textContent=sf.length?Math.max(...sf):'—';try{const stats=await data.loadInsightStats(sb,user.id),holes=(stats.holes||[]).filter(h=>h.course_hole);if(!holes.length){$('#insights').innerHTML='<div class="muted">Vul tijdens je rondes enkele extra stats in. Daarna laat BOUNDS zien waar je structureel de meeste winst kunt pakken.</div>';return}const byPar={3:[],4:[],5:[]};holes.forEach(h=>{const par=Number(h.course_hole.par),score=Number(h.score);if(byPar[par]&&Number.isFinite(score))byPar[par].push(score-par)});const parSummary=Object.entries(byPar).filter(([,v])=>v.length).map(([p,v])=>({p:Number(p),avg:v.reduce((a,b)=>a+b,0)/v.length,n:v.length})).sort((a,b)=>b.avg-a.avg),worstPar=parSummary[0];const putts=holes.map(h=>Number(h.putts)).filter(Number.isFinite),gir=holes.map(h=>h.gir).filter(v=>v==='yes'||v==='no'),fw=holes.filter(h=>[4,5].includes(Number(h.course_hole.par))).map(h=>h.fairway).filter(v=>v==='yes'||v==='no'),penalties=holes.map(h=>Number(h.penalty)).filter(Number.isFinite),avgPutts=putts.length?putts.reduce((a,b)=>a+b,0)/putts.length:null,girPct=gir.length?gir.filter(v=>v==='yes').length/gir.length:null,fwPct=fw.length?fw.filter(v=>v==='yes').length/fw.length:null,totalPenalties=penalties.reduce((a,b)=>a+b,0);const cards=[];if(worstPar)cards.push(`<div class="insight-card"><div class="insight-kicker">GROOTSTE KANS</div><b>Par ${worstPar.p}</b><span>Gemiddeld ${worstPar.avg>=0?'+':''}${worstPar.avg.toFixed(1).replace('.',',')} slag t.o.v. par over ${worstPar.n} gespeelde holes.</span></div>`);if(avgPutts!==null)cards.push(`<div class="insight-card"><div class="insight-kicker">PUTTING</div><b>${avgPutts.toFixed(1).replace('.',',')} putts</b><span>${avgPutts>=2?'Dit is een aandachtspunt.':'Dit ziet er op basis van de ingevulde data stabiel uit.'}</span></div>`);if(girPct!==null)cards.push(`<div class="insight-card"><div class="insight-kicker">GIR</div><b>${Math.round(girPct*100)}%</b><span>Gebaseerd op ${gir.length} holes met GIR ingevuld.</span></div>`);if(fwPct!==null)cards.push(`<div class="insight-card"><div class="insight-kicker">FAIRWAY</div><b>${Math.round(fwPct*100)}%</b><span>Alleen par 4 en par 5 tellen mee.</span></div>`);if(totalPenalties)cards.push(`<div class="insight-card"><div class="insight-kicker">PENALTIES</div><b>${totalPenalties}</b><span>Geregistreerde penaltyslagen. Dit is directe score-impact.</span></div>`);const recent=stats.rounds.slice(0,6).reverse().filter(r=>r.players?.[0]?.final_score!=null),trend=recent.map(r=>`<div class="trend-row"><span>${dateLabel(r.played_at)}</span><b>${r.players[0].final_score}</b></div>`).join(''),coverage=`${holes.length} holes met scoredata · ${putts.length} met putts · ${gir.length} met GIR · ${fw.length} met fairway · ${penalties.length} met penalty`;$('#insights').innerHTML=`<div class="insight-lead"><b>Waar zit je grootste winst?</b><span>BOUNDS kijkt eerst naar score boven par en gebruikt extra stats alleen wanneer je ze invult.</span></div><div class="insight-grid">${cards.join('')}</div><div class="insight-subsection"><div class="section-title">Score per par-type</div>${parSummary.map(x=>`<div class="performance-row"><b>Par ${x.p}</b><span>${x.avg>=0?'+':''}${x.avg.toFixed(1).replace('.',',')} / hole</span><small>${x.n} holes</small></div>`).join('')}</div>${recent.length?`<div class="insight-subsection"><div class="section-title">Laatste scores</div>${trend}</div>`:''}<div class="stat-coverage">${coverage}</div>`}catch(e){console.error('My Game insights:',e);$('#insights').innerHTML='<div class="muted">De basisstatistieken zijn beschikbaar, maar de detailanalyse kon nu niet worden geladen.</div>'}}
async function openRound(roundId){try{const r=await data.loadRoundDetail(sb,user.id,roundId);if(!r){toast('Ronde niet gevonden.');return}openStatsHoles.clear();activeRound={clientRoundId:r.client_round_id||crypto.randomUUID(),courseId:r.course_id,teeId:r.tee_id,holesPlayed:Number(r.holes_played),loop:r.loop||'full',handicap:Number(r.player.handicap_index_at_round),courseHandicap:Number(r.player.course_handicap),holes:r.holes};$('#hcpInput').value=activeRound.handicap;$('#courseSelect').value=activeRound.courseId;await loadCourseConfiguration();$('#holesSelect').value=String(activeRound.holesPlayed);await loadCourseConfiguration();$('#variantSelect').value=tees.find(t=>t.id===activeRound.teeId)?.course_variant||'';await loadTeeOptions();$('#teeSelect').value=activeRound.teeId;await updateRoundConfig();$('#roundSetup').classList.add('hidden');$('#scoreArea').classList.remove('hidden');$('#roundStatus').classList.remove('hidden');$('#scoreSubtitle').textContent=`${r.course?.name||'Baan'} · ${r.holes_played} holes · ${dateLabel(r.played_at)}`;renderScorecard();persistDraft();showTab('play')}catch(e){console.error(e);toast('Ronde kon niet worden geopend.')}}
async function renderCourses(){const h=await data.loadCourseHistory(sb,user.id);const by=new Map();for(const r of h){if(!by.has(r.course_id))by.set(r.course_id,[]);by.get(r.course_id).push(r)}$('#courseCatalog').innerHTML=courses.map(c=>{const rs=by.get(c.id)||[],scores=rs.flatMap(r=>r.players||[]).map(p=>Number(p.final_score)).filter(Number.isFinite),avg=scores.length?(scores.reduce((a,b)=>a+b,0)/scores.length).toFixed(1):'—';return `<button class="course-item course-button" data-course="${c.id}"><div><b>${esc(c.name)}</b><small>${esc(c.location||'Nederland')} · ${rs.length} ${rs.length===1?'ronde':'rondes'}</small></div><div><b>${scores.length?Math.min(...scores):'—'}</b><small>${avg==='—'?'':'gem. '+avg}</small></div></button>`}).join('');$$('[data-course]').forEach(b=>b.onclick=()=>openCourseDetail(b.dataset.course))}
async function openCourseDetail(id){const c=courses.find(x=>x.id===id);if(!c)return;const rs=(await data.loadCourseHistory(sb,user.id)).filter(r=>r.course_id===id),all=await data.loadTees(sb,id,18).catch(()=>[]),nine=await data.loadTees(sb,id,9).catch(()=>[]),allTees=[...all,...nine],vars=[...new Set(allTees.map(t=>t.course_variant).filter(Boolean))];$('#courseCatalog').innerHTML=`<div class="course-detail"><button class="text-button" id="backCourses">← Alle banen</button><div class="eyebrow">COURSE</div><h3>${esc(c.name)}</h3><div class="muted">${esc(c.location||'Nederland')} · ${esc(c.country||'NL')}</div><div class="course-facts"><div><b>${vars.length?vars.map(variantLabel).join(' · '):'—'}</b><span>Layouts</span></div><div><b>${rs.length}</b><span>Jouw rondes</span></div><div><b>${rs.length?Math.min(...rs.flatMap(r=>r.players||[]).map(p=>Number(p.final_score)).filter(Number.isFinite)):'—'}</b><span>Beste score</span></div></div><div class="section-title">Jouw historie</div>${rs.length?rs.slice(0,10).map(roundHistoryItem).join(''):'<div class="muted">Nog geen rondes op deze baan.</div>'}</div>`;$('#backCourses').onclick=renderCourses;$$('[data-round]').forEach(b=>b.onclick=()=>openRound(b.dataset.round));$$('[data-delete-round]').forEach(b=>b.onclick=(e)=>{e.stopPropagation();deleteSavedRound(b.dataset.deleteRound)})}
function wireAuthForm(){$$('.tab').forEach(b=>b.onclick=()=>showTab(b.dataset.tab));$$('[data-go]').forEach(b=>b.onclick=()=>showTab(b.dataset.go));$$('[data-new-round]').forEach(b=>b.onclick=startNewRound);$('#cancelRound')?.addEventListener('click',cancelRound);$('#cancelRoundTop')?.addEventListener('click',cancelRound);['#authEmail','#authPassword'].forEach(sel=>{const el=$(sel);if(el){el.disabled=false;el.readOnly=false;el.tabIndex=0;el.style.pointerEvents='auto'}});$('#authMode').onclick=()=>{authRegister=!authRegister;$('#authSubmit').textContent=authRegister?'Account maken':'Inloggen';$('#authMode').textContent=authRegister?'Al een account? Inloggen':'Nog geen account? Registreren';setMessage('')};$('#authForm').onsubmit=async e=>{e.preventDefault();setMessage('');const email=$('#authEmail').value.trim(),password=$('#authPassword').value;if(!email||!password){setMessage('Vul e-mail en wachtwoord in.');return}const button=$('#authSubmit');button.disabled=true;try{await waitForSupabase();if(!sb?.auth)throw new Error('Supabase Auth is niet beschikbaar.');if(authRegister){const {data,error}=await sb.auth.signUp({email,password});if(error)throw error;if(data.session){await enterApp(data.session.user);toast('Account aangemaakt.')}else setMessage('Account aangemaakt. Controleer je e-mail als verificatie actief is.')}else{const {data,error}=await sb.auth.signInWithPassword({email,password});if(error)throw error;if(data?.session?.user)await enterApp(data.session.user);else throw new Error('Geen actieve sessie ontvangen na inloggen.')}}catch(err){console.error('Auth error',err);const msg=String(err?.message||'Inloggen mislukt.');const friendly=/invalid login credentials/i.test(msg)?'Inloggen mislukt. Controleer e-mailadres en wachtwoord.':/email not confirmed/i.test(msg)?'Bevestig eerst je e-mailadres via de bevestigingsmail.':msg;setMessage(friendly)}finally{button.disabled=false}}}
$$('.tab').forEach(b=>b.onclick=()=>showTab(b.dataset.tab));boot();
