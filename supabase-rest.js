...
// GitHub Pages uses direct Supabase REST/Auth calls so login does not depend
// on a third-party browser bundle loading correctly in Safari.
export function createBoundsSupabase(url, anonKey){
  ...
  async function request(base,path='',options={}){
    ...
    try{
      ...
      if(!res.ok){
        const e=new Error(body?.msg||body?.message||body?.error_description||body?.error||text||`HTTP ${res.status}`);
        e.status=res.status;e.code=body?.code;throw e;
      }
      return body;
    }catch(e){
      if(e?.name==='AbortError') throw new Error('De verbinding met Supabase duurt te lang. Probeer opnieuw.');
      throw e;
  ...
  const auth={
    async getSession(){
      let s=read();
      if(!s)return result({session:null});
      if(s.expires_at&&Date.now()/1000>Number(s.expires_at)-30&&s.refresh_token){
        try{
          ...
          write(null);s=null;listeners.forEach(f=>f('SIGNED_OUT',null));
        }
      }
    ...
    async signInWithPassword({email,password}){
      try{
        ...
        write(s);listeners.forEach(f=>f('SIGNED_IN',s));
  ...
  return {
    auth,
    from:t=>new Query(t),
    rpc:async(name,payload={})=>{try{return result(await request(REST,`/rpc/${name}`,{method:'POST',body:JSON.stringify(payload)}))}catch(e){return result(null,e)}}
  };
}
