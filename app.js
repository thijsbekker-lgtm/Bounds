...
function wireAuthForm(){$$('.tab').forEach(b=>b.onclick=()=>showTab(b.dataset.tab));$$('[data-go]').forEach(b=>b.onclick=()=>showTab(b.dataset.go));$$('[data-new-round]').forEach(b=>b.onclick=startNewRound);$('#cancelRound')?.addEventListener('click',cancelRound);$('#cancelRoundTop')?.addEventListener('click',cancelRound);['#authEmail','#authPassword'].forEach(sel=>{const el=$(sel);if(el){el.disabled=false;el.readOnly=false;el.tabIndex=0;el.style.pointerEvents='auto'}});$('#authMode').onclick=()=>{authRegister=!authRegister;$('#authSubmit').textContent=authRegister?'Account maken':'Inloggen';$('#authMode').textContent=authRegister?'Al een account? Inloggen':'Nog geen account? Registreren';setMessage('')};$('#authForm').onsubmit=async e=>{e.preventDefault();const email=$('#authEmail').value.trim(),password=$('#authPassword').value;if(!email||!password){setMessage('Vul e-mail en wachtwoord in.');return}setMessage(authRegister?'Account maken…':'Inloggen…');$('#authSubmit').disabled=true;try{const client=await waitForSupabase();const res=authRegister?await client.auth.signUp({email,password}):await client.auth.signInWithPassword({email,password});if(res.error)throw res.error;if(authRegister&&!res.data?.session){setMessage('Account aangemaakt. Controleer je e-mail om je account te bevestigen.');return}if(res.data?.session?.user)await enterApp(res.data.session.user);else if(res.data?.user)await enterApp(res.data.user);else throw new Error('Geen geldige sessie ontvangen.')}catch(err){console.error('BOUNDS login error',err);setMessage(err?.message||'Inloggen mislukt.')}finally{$('#authSubmit').disabled=false}}}
