...
async function sfoPatchCourse(){try{const s=sfoSession();if(!s?.user?.id)return;const rows=await sfoRest(`golf_flight_players?select=flight_id,status,created_at&user_id=eq.${encodeURIComponent(s.user.id)}&status=in.(accepted,declined)&order=created_at.desc&limit=1`);const row=rows?.[0];if(!row)return;const flights=await sfoRest(`golf_flights?select=course_id& id=eq.${encodeURIComponent(row.flight_id)}`.replace('?select=course_id& id','?select=course_id&id'));const courseId=flights?.[0]?.course_id;if(!courseId)return;const courses=await sfoRest(`courses?select=id,name&id=eq.${encodeURIComponent(courseId)}&limit=1`);const name=courses?.[0]?.name;if(!name)return;const span=document.querySelector('.sfr-notice .sfr-notice-copy span');if(!span)return;const parts=span.textContent.split(' · ');if(parts.length<4)return;parts[1]=name;span.textContent=parts.join(' · ')}catch(e){console.debug('BOUNDS flight outcome course name',e)}}
