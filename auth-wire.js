...
window.wireAuthForm=function wireAuthForm(){
  ...
  form.addEventListener('submit',async event=>{
    ...
    try{
      ...
      const response=await fetch(`${BOUNDS_AUTH_URL}/auth/v1${endpoint}`,{
        method:'POST',
        headers:{apikey:BOUNDS_AUTH_KEY,'Content-Type':'application/json'},
      ...
      if(!data.access_token){
        ...
        registering=false;button.disabled=false;button.textContent='Inloggen';
        return;
      }
