...
async function loadTeeOptions(){
  ...
  $('#teeSelect').innerHTML=unique.map(t=>`<option value="${t.id}">${esc(t.tee_name)} · CR ${fmt(t.course_rating)} / ${t.slope_rating}</option>`).join('')||'<option value="">Geen tee beschikbaar</option>';
...
function renderScorecard(){
  if(!activeRound)return; const n=activeRound.holes.length; const indices=activeRound.holes.map(h=>h.si);
  $('#scoreRows').innerHTML=activeRound.holes.map((h,i)=>{const strokes=strokesForSI(activeRound.courseHandicap,h.si,n,indices);const sf=stableford(h.par,h.score,strokes);h.sf=sf;return `<div class="hole" data-i="${i}"><div class="hole-main"><div class="hole-no"><b>${h.hole}</b><small>Par ${h.par} · SI ${h.si}</small></div><div class="muted">${strokes?`+${strokes} slag${strokes>1?'en':''}`:'Geen slag'}</div><div class="scorebox"><button data-act="minus">−</button><span class="score">${h.score||'—'}</span><button data-act="plus">+</button></div><div class="sf">${sf??'—'} SF</div></div><div class="stats-panel"><div class="extra-grid"><label>Putts<div class="stepper"><button data-extra="putts:-">−</button><span>${h.putts||'—'}</span><button data-extra="putts:+">+</button></div></label><label>Penalty<div class="stepper"><button data-extra="penalty:-">−</button><span>${h.penalty||'—'}</span><button data-extra="penalty:+">+</button></div></label><label>Fairway<select data-field="fairway"><option value="" ${!h.fairway?'selected':''}>Niet ingevuld</option><option value="hit" ${h.fairway==='hit'?'selected':''}>✓ geraakt</option><option value="miss" ${h.fairway==='miss'?'selected':''}>✕ gemist</option></select></label><label>GIR<select data-field="gir"><option value="" ${!h.gir?'selected':''}>Niet ingevuld</option><option value="yes" ${h.gir==='yes'?'selected':''}>✓ GIR</option><option value="no" ${h.gir==='no'?'selected':''}>✕ geen GIR</option></select></label></div><button class="text-button note-btn" data-note>📝 ${h.note?'Notitie bekijken/wijzigen':'Notitie toevoegen'}</button></div></div>`}).join('');
  $('#scoreRows').onclick=e=>{const row=e.target.closest('.hole');if(!row)return;const i=Number(row.dataset.i),h=activeRound.holes[i];if(e.target.dataset.act){const d=e.target.dataset.act==='plus'?1:-1;h.score=h.score?Math.max(1,h.score+d):h.par;renderScorecard();persistDraft()}if(e.target.dataset.extra){const [field,op]=e.target.dataset.extra.split(':');h[field]=String(Math.max(0,Number(h[field]||0)+(op==='+'?1:-1))||'');renderScorecard();persistDraft()}if(e.target.dataset.note){const v=prompt(`Notitie voor hole ${h.hole}:`,h.note||'');if(v!==null){h.note=v;renderScorecard();persistDraft()}}};
  $('#scoreRows').onchange=e=>{const row=e.target.closest('.hole');if(!row)return;const h=activeRound.holes[Number(row.dataset.i)];if(e.target.dataset.field)h[e.target.dataset.field]=e.target.value;persistDraft()};
  const totals=roundTotals(activeRound.holes);$('#totalScore').textContent=totals.score;$('#totalSf').textContent=`${totals.stableford} SF`;
}
function persistDraft(){localStorage.setItem(localKey,JSON.stringify({clientRoundId:activeRound.clientRoundId,scores:Object.fromEntries(activeRound.holes.map(h=>[h.hole,h.score])),extra:Object.fromEntries(activeRound.holes.map(h=>[h.hole,{putts:h.putts,penalty:h.penalty,fairway:h.fairway,gir:h.gir,note:h.note}]))}))}
