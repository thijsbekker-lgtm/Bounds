...
async function load(force=false){
  ...
  try{
    ...
    const metrics=[avgPutts!==null?metric('Putts',fmt(avgPutts),'Gemiddeld per hole met putts ingevuld.'):null,girPct!==null?metric('GIR',`${Math.round(girPct)}%`,`${gir.length} holes ingevuld.`):null,fwPct!==null?metric('Fairway',`${Math.round(fwPct)}%`,`${fairways.length} par 4/5 holes ingevuld.`):null,metric('Penalty',String(penaltyTotal),penaltyTotal?'Geregistreerde penaltyslagen.':'Geen penaltyslagen geregistreerd.')].filter(Boolean).join('');
  ...
  }catch(e){console.error('BOUNDS My Game enhance',e);if(insights)insights.classList.add('ready');}finally{running=false}
}

window.addEventListener('load',()=>{setTimeout(()=>load(),0)});
document.addEventListener('click',e=>{if(e.target.closest('.tab[data-tab="game"]'))setTimeout(()=>load(true),0)});
