...
// One deterministic browser adapter for GitHub Pages.
export function createBoundsSupabase(url, anonKey){
  ...
  const networkError=e=>{
    ...
    return !navigator.onLine||e?.name==='TypeError'||e?.name==='AbortError'||e?.name==='TimeoutError'||e?.status===408||e?.status===504||/Failed to fetch|NetworkError|Load failed|network|timed? ?out|timeout|aborted/i.test(msg);
  };
  ...
  async function flushQueue(){
    const queue=readQueue();if(!queue.length||!navigator.onLine)return;
    let firstUnprocessed=queue.length;
    for(let i=0;i<queue.length;i++){
      const item=queue[i];
      try{await request(REST,`/rpc/${item.name}`,{method:'POST',body:JSON.stringify(item.payload)})}
      catch(e){if(networkError(e)){firstUnprocessed=i;break}console.error('BOUNDS sync item rejected',e)}
    }
    if(firstUnprocessed<queue.length)writeQueue(queue.slice(firstUnprocessed));else writeQueue([]);
  }
  class Query{
  ...
  const auth={
    async getSession(){
      ...
      if(s.expires_at&&Date.now()/1000>Number(s.expires_at)-30&&s.refresh_token){
        try{
          const r=await request(AUTH,'/token?grant_type=refresh_token',{method:'POST',skipSessionAuth:true,body:JSON.stringify({refresh_token:s.refresh_token})});
          s={...s,...r,expires_at:Math.floor(Date.now()/1000)+Number(r.expires_in||3600)};write(s);
    ...
    async signInWithPassword({email,password}){
      try{
        const d=await request(AUTH,'/token?grant_type=password',{method:'POST',skipSessionAuth:true,body:JSON.stringify({email,password})});
        const s={...d,expires_at:Math.floor(Date.now()/1000)+Number(d.expires_in||3600)};write(s);listeners.forEach(f=>f('SIGNED_IN',s));
        return result({session:s,user:d.user});
      }catch(e){return result({session:null},e)}
    },
    async signUp({email,password}){
      try{
        const d=await request(AUTH,'/signup',{method:'POST',skipSessionAuth:true,body:JSON.stringify({email,password})});
        let s=null;
        if(d?.access_token){s={...d,expires_at:Math.floor(Date.now()/1000)+Number(d.expires_in||3600)};write(s);listeners.forEach(f=>f('SIGNED_IN',s))}
        return result({session:s,user:d?.user||null});
      }catch(e){return result({session:null},e)}
    },
    async signOut(){
      const s=read();
      try{if(s?.access_token)await request(AUTH,'/logout',{method:'POST'})}catch{}
      write(null);listeners.forEach(f=>f('SIGNED_OUT',null));return result(null);
    }
  };
