...
(function(){
  ...
  function render(){
    ...
    const hcp=Number.isFinite(Number(p.handicap_index))?p.handicap_index:14.2;
    ...
    host.innerHTML=`<div class="profile-pref-card card"><div class="eyebrow">PROFIEL</div><h3>Jouw golfvoorkeuren</h3><div class="profile-pref-grid">
      ...
      <label>Tee selectie<select id="profileTeeGender"><option value="men" ${gender==='men'?'selected':''}>Heren</option><option value="women" ${gender==='women'?'selected':''}>Dames</option></select></label>
      ...
      <label>Speelstijl<select id="profilePlayingStyle"><option value="">Geen voorkeur</option><option value="rustig" ${style==='rustig'?'selected':''}>Rustig</option><option value="sociaal" ${style==='sociaal'?'selected':''}>Sociaal</option><option value="sportief" ${style==='sportief'?'selected':''}>Sportief</option><option value="competitief" ${style==='competitief'?'selected':''}>Competitief</option></select></label>
    ...
    $('#saveProfilePreferences').onclick=async()=>{
      ...
      try{
        ...
        const teeGender=$('#profileTeeGender').value;const teeColor=$('#profileTeeColor').value||null;const playingStyle=$('#profilePlayingStyle').value||null;
        await request(`/profiles?id=eq.${encodeURIComponent(currentId)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({handicap_index:h,home_course_id:homeId,tee_gender_preference:teeGender,tee_color_preference:teeColor,playing_style:playingStyle})});
        ...
        if(homeId&&!existingByCourse.has(String(homeId))){try{await request('/user_courses',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({user_id:currentId,course_id:homeId,is_member:false,is_home_course:true,is_favorite:false})})}catch(e){console.warn('user_courses relation could not be created',e)}}
        try{userCourses=await request(`/user_courses?select=id,course_id,is_member,is_home_course,is_favorite&user_id=eq.${encodeURIComponent(currentId)}`)||userCourses}catch(e){}
        window.boundsProfile={...window.boundsProfile,handicap_index:h,home_course_id:homeId,tee_gender_preference:teeGender,tee_color_preference:teeColor,playing_style:playingStyle};
        ...
        $('#profileMessage').textContent='Opgeslagen';document.dispatchEvent(new CustomEvent('bounds:profile-updated',{detail:window.boundsProfile}));
