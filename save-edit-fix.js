...
// Restore round identity before saving so edits stay idempotent.
...
const BOUNDS_SAVE_SESSION_KEY='bounds_supabase_session';
...
async function findExistingRound(clientRoundId){
  const session=readJson(BOUNDS_SAVE_SESSION_KEY);
  ...
  try{
    const res=await nativeFetch(`${BOUNDS_SUPABASE_URL}/rest/v1/rounds?${qs}`,{headers:{apikey:BOUNDS_SUPABASE_KEY,Authorization:`Bearer ${session.access_token}`}});
...
window.fetch=async(input,init={})=>{
  ...
  if(!UUID.test(String(payload.p_client_round_id||''))&&UUID.test(String(draft?.clientRoundId||'')))payload.p_client_round_id=draft.clientRoundId;
